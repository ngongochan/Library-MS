...
FictionBook::FictionBook(std::string title, std::string author,
...
void FictionBook::addToWaitlist(
    Member* member) {  // takes address of member (&member)
  // add member object to waitlist
  waitlist.push_back(member);
}

bool FictionBook::get_isAvailable() { return isAvailable; }

void FictionBook::set_isAvailable(bool b) { isAvailable = b; }

void FictionBook::displayInfo() {
...
std::vector<Member*> FictionBook::get_waitlist() { return waitlist; }

FictionBook::~FictionBook() { waitlist.clear(); }